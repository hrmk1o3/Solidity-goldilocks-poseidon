...
fn mds_row_shf(r: usize, v: &[F; WIDTH]) -> F {
    debug_assert!(r < WIDTH);
    let mut res = F::ZERO;
    for i in 0..12 {
        res += v[(i + r) % WIDTH] * F::from_canonical_u64(MDS_MATRIX_CIRC[i]);
    }
    res += v[r] * F::from_canonical_u64(MDS_MATRIX_DIAG[r]);
    res
}
...
fn partial_rounds_naive(state: &mut [F; WIDTH], round_ctr: &mut usize) {
...
fn poseidon_naive(input: [F; WIDTH]) -> [F; WIDTH] {
    ...
    partial_rounds_naive(&mut state, &mut round_ctr);
    full_rounds(&mut state, &mut round_ctr);
...
impl PlonkyPermutation<F> for PoseidonPermutation {
    fn permute(input: [F; SPONGE_WIDTH]) -> [F; SPONGE_WIDTH] {
        poseidon_naive(input)
    }
}
